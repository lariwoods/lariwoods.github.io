# Olá! Sou Larissa 👋

Sou estudante de Análise e Desenvolvimento de Sistemas na UNICID, com interesse em tecnologia e desenvolvimento de software.

## Sobre mim

Atualmente, estou construindo minha trajetória na área de tecnologia por meio da graduação, projetos acadêmicos e estudos. Tenho desenvolvido conhecimentos em programação, Inteligência Artificial, Engenharia de Prompt e prototipagem de interfaces.

Busco ampliar continuamente minhas habilidades técnicas e adquirir experiência prática, explorando diferentes possibilidades dentro da tecnologia e construindo uma base sólida para minha carreira.

## Projetos

### PetCare Manager

Projeto acadêmico voltado à criação de uma solução digital para uma clínica veterinária e pet shop. Participei do levantamento de requisitos, análise de processos, modelagem BPMN e UML e elaboração de protótipos.

[Ver projeto](LINK_DO_REPOSITORIO)

### Dubai Express

Projeto acadêmico desenvolvido na disciplina de Engenharia de Prompt e Inteligência Artificial. Desenvolvi, em equipe, uma plataforma de viagens sobre Dubai utilizando ferramentas de IA e aprimoramento iterativo de prompts.

[Ver projeto](LINK_DO_REPOSITORIO)

### Biblioteca+ Acessível

Projeto desenvolvido no tema “Design para Todos — Criando uma Solução Digital Inclusiva”, com foco em acessibilidade e experiência do usuário. Participei da estruturação da solução, personas, arquitetura da informação, user flow e prototipagem.

[Ver projeto](LINK_DO_REPOSITORIO)

## Habilidades

- Python
- Inteligência Artificial
- Engenharia de Prompt
- Prototipagem de Interfaces
- UML

## Formação

**Análise e Desenvolvimento de Sistemas**  
UNICID — Universidade Cidade de São Paulo  
2026–2027

## Contato

- LinkedIn: [Meu LinkedIn]([LINK_DO_LINKEDIN](https://www.linkedin.com/in/larissa-dos-santos-soares-855076417/))
- GitHub: [@lariwoods]([LINK_DO_GITHUB](https://github.com/lariwoods/lariwoods.github.io))
- E-mail: lari.soares100a@gmail.com
